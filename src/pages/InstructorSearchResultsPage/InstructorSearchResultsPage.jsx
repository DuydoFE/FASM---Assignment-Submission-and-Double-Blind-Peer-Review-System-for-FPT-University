...
const InstructorSearchResultsPage = () => {
  ...
  const handleCardClick = (item) => {
    ...
    if (item.type === "Assignment" || item.type === "assignment") {
      if (item.classId && item.assignmentId) {
        navigate(`/instructor/class/${item.classId}/assignment/${item.assignmentId}`);
      } else if (item.classId) {
        navigate(`/instructor/class/${item.classId}`);
      } else {
        console.warn("Missing classId or assignmentId for navigation");
      }
    } else if (item.type === "Feedback" || item.type === "feedback") {
      if (item.submissionId) {
        navigate(`/instructor/submission/${item.submissionId}`);
      }
      ...
      if (item.rubricId) {
        navigate(`/instructor/rubric/${item.rubricId}`);
      }
    }
  };
  ...
  const renderList = (dataSource, icon, renderItemContent) => {
    if (!dataSource || dataSource.length === 0) {
      return <Empty description={<span className="text-gray-500">No results found</span>} />;
    }
  ...
  const items = [
    {
      ...
      children: renderList(data?.assignments, <BookOpen />, (item) => (
        <>
          <Title level={5} className="!text-gray-800 !mb-2">{item.title}</Title>
          <Text className="text-gray-600 block mb-1">Class: {item.className}</Text>
          <Text className="text-gray-500 text-sm line-clamp-3">{item.descriptionSnippet}</Text>
        </>
      )),
    },
    {
      ...
      children: renderList(data?.feedback, <MessageSquare />, (item) => (
        <>
          <Title level={5} className="!text-gray-800 !mb-2">For: {item.assignmentTitle}</Title>
          <Text className="text-gray-600 block mb-1">Student: {item.studentName}</Text>
          <Tag color={item.reviewerType === 'Instructor' ? 'gold' : 'blue'} className="mb-2">{item.reviewerType}</Tag>
          <Text className="text-gray-500 block italic">"{item.overallFeedback}"</Text>
        </>
      )),
    },
    {
      ...
      children: renderList(data?.submissions, <CheckCircle />, (item) => (
        <>
           <Title level={5} className="!text-gray-800 !mb-2">{item.assignmentTitle}</Title>
           <Text className="text-gray-600 block">Student: {item.studentName}</Text>
           <Text className="text-gray-600 block">File: {item.fileName}</Text>
           <Text className="text-gray-500 block text-xs mt-1">Keywords: {item.keywords}</Text>
           <Text className="text-gray-500 block text-xs">Submitted: {new Date(item.submittedAt).toLocaleDateString()}</Text>
        </>
      )),
    },
    {
      ...
      children: renderList(data?.summaries, <FileText />, (item) => (
        <>
          ...
          <Text className="text-gray-600 block mt-2 text-sm">{item.contentSnippet}</Text>
          <Text className="text-gray-500 text-xs mt-2 block">Generated: {new Date(item.generatedAt).toLocaleDateString()}</Text>
        </>
      )),
    },
    {
      ...
      children: renderList(data?.criteria, <ListChecks />, (item) => (
        <>
           <Title level={5} className="!text-gray-800 !mb-2">{item.title}</Title>
           <Text className="text-gray-600 block text-sm">{item.description}</Text>
